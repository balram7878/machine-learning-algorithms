...
# Load dataset (10 classes, 64 features, 1797 samples)
...
print(f"Predictions where KNN and DT disagree: {diff_count} out of {len(y_test)}")

# Bar chart comparison
plt.figure(figsize=(6, 4))
plt.bar(['KNN (k=5)', 'Decision Tree'], [float(knn_acc), float(dt_acc)], color=['steelblue', 'coral'])
plt.ylabel('Accuracy')
plt.title('KNN vs Decision Tree (Digits Dataset)')
plt.ylim(0.7, 1.0)
plt.tight_layout()
plt.show()
